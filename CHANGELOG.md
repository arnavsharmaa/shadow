...
- `PATCH /api/v1/traces/:traceId` and `shadow traces update`: rename a trace, add, remove or
  ...
  immediately.
- The web app detects agents without a registered program (for example OpenTelemetry imports):
  the trace header shows a "not replayable" badge and the fork dialog creates the branch with
  its overrides for inspection instead of failing at replay.
...
- Notes on events: the event inspector has an "Add a note" box (Ctrl+Enter saves) and renders
  notes as text; `shadow artifacts add <traceId> --kind note --event <id> --content ...` stores
  notes or any document from the CLI (`--file` reads content from disk).
...
- `GET /metrics`: Prometheus exposition of HTTP request counts and latencies, traces created,
  events ingested by source, forks, replays by status, comparisons, OTLP requests, pruned traces
  and storage gauges. Protected by the bearer token when one is configured.
- Comparison history: a "Comparisons" tab on the trace detail lists saved comparisons with
  their outcome change and first divergence, and `shadow comparisons list <traceId>` /
  `shadow comparisons show <comparisonId>` do the same from the CLI.
